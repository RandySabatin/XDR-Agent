...
EventMonitor::StartEventMonitor()
{
    ...
    if (NULL == hSubscription)
    {
        ...
        if (ERROR_EVT_CHANNEL_NOT_FOUND == status)
            LOG(Logger::ERRORS, L"Failed to subscribe to event log channel. The specified event channel was not found.");
            ...
            LOG(Logger::ERRORS, Utility::FormatString(L"Failed to subscribe to event log channel. Error: %lu.", status).c_str()
            );
    }
    ...
    )
{
    ...
    {
		if (!DoesChannelExist(eventChannel.Name))
		{
			LOG(Logger::ERRORS, Utility::FormatString(
				L"Event channel %s does not exist. Skipping it.",
				eventChannel.Name.c_str()).c_str()
			);
			continue;
		}
        //
...
/// 
/// 
bool EventMonitor::DoesChannelExist(const std::wstring& channelName)
{
    EVT_HANDLE h = EvtOpenChannelConfig(nullptr, channelName.c_str(), 0);
    if (h == nullptr)
        return false;
...
Exit:
    ...
    if (ERROR_SUCCESS != status) {
        LOG(Logger::INFO, Utility::FormatString(L"Waiting for %ws event channel to be enabled",
            ChannelPath).c_str());
    }
